	...
	function wdf_pledges_panel($echo = true, $post_id = '', $context = '', $args = array() ) {
		...
		if($donations) {
			...
			foreach($donations_ready as $donation) {
				...
				$count ++;
				$content .= '<li>'.esc_html($donation->trans['first_name']).' '.esc_html(substr($donation->trans['last_name'], 0, 1)).' - '.$wdf->format_currency('',$donation->trans['gross']).'</li>';
			}
			$content .= '</ul>';
...
if(!function_exists('wdf_time_left')) {
	function wdf_time_left($echo = true, $post_id = '', $active_bool = false ) {
		...
		if((int)$days >= 2) {
			$time = ((int)$days == 1 ? __('Day Left','wdf') : __('Days Left','wdf')) . ': ' . $days;
		} elseif((int)$hours < 1) {
			$time = ((int)$min == 1 ? __('Minute Left','wdf') : __('Minutes Left','wdf')) . ': ' . $min;
		} else {
			$time = ((int)$hours == 1 ? __('Hour Left','wdf') : __('Hours Left','wdf')) . ': ' . $hours;
		}
...
if(!function_exists('wdf_confirmation_page')) {
	function wdf_confirmation_page( $echo = true, $post_id = '' ) {
		...
		$pledge_id = (isset($_SESSION['wdf_pledge_id']) ? $_SESSION['wdf_pledge_id'] : (isset($_REQUEST['pledge_id']) && isset($_REQUEST['status']) && $_REQUEST['status'] == 'OK' ? $_REQUEST['pledge_id'] : ''));
				...
				$content .= apply_filters('wdf_before_rewards_title','');
					...
					foreach($level as $index => $data) {
						...
						<div class="wdf_reward_item'.$disabled_class.'">
							...
							<div class="wdf_reward_description">'.wp_kses_post(html_entity_decode($data['description'])).'</div>
						</div>';
					}
					$content .= '
...
if(!function_exists('wdf_pledge_button')) {
	function wdf_pledge_button($echo = true, $context = '', $post_id = '', $args = array()) {
		...
		if($context == 'widget_simple_donate') {
			...
			$content .= '<label for="firstname" class="wdf_first_name">' . __('First Name', 'wdf') . ':</label><br />';
            $content .= '<input type="text" class="wdf_first_name" name="firstname" value="' . (isset($_POST['firstname']) ? esc_attr($_POST['firstname']) : '') . '" required/><br />';
            $content .= '<label for="lastname" class="wdf_last_name">' . __('Last Name', 'wdf') . ':</label><br />';
			$content .= '<input type="text" class="wdf_last_name" name="lastname" value="' . (isset($_POST['lastname']) ? esc_attr($_POST['lastname']) : '') . '" required/><br />';
            $content .= '<label for="e-mail" class="wdf_email">' . __('E-mail', 'wdf') . ':</label><br />';
            $content .= '<input type="text" class="wdf_email" name="e-mail" value="' . (isset($_POST['e-mail']) ? esc_attr($_POST['e-mail']) : '') . '" required/>';
			$content .= '<input type="hidden" name="funder_id" value="'.$post_id.'" />';
			$content .= '<input id="channel" type="hidden" name="channel" value="" />';
			...
			$content .= ($settings['curr_symbol_position'] == 1 || $settings['curr_symbol_position'] == 2 ? '<span class="currency">'.$wdf->format_currency().' </span>' : '');
			$content .= '<input type="text" name="wdf_pledge" class="wdf_pledge_amount" value="" required/> ';
			$content .= ($settings['curr_symbol_position'] == 3 || $settings['curr_symbol_position'] == 4 ? '<span>'.$wdf->format_currency().' </span>' : '');
			$content .= '<div class="privacy-checkbox"><input type="checkbox" name="wdf_privacy" class="wdf_pledge_privacy" value="accepted" required/><span class="currency">Akceptuję <a href="'.home_url( '/polityka-prywatnosci' ).'" target="_blank">politykę prywatności</a></span></div>';
